...
exports.sendCurrentUser = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    if (decodedData.exp <= nowInSeconds) {
      // Token has expired, generate a new token and send it
      const newToken = jwt.sign({ id: admin._id }, process.env.JWT_SECRET, {
        expiresIn: '1h', // Set the expiration time for the new token
      });
    ...
    // Token is valid, send the current user data
    sendToken(admin, 200, res);
  } catch (err) {
